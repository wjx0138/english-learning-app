...
class _EnhancedFlashcardPageState extends State<EnhancedFlashcardPage> {
  ...
  late CardProvider _cardProvider; // Save reference for safe use in dispose
  ...
  void _loadNextCard() {
    if (_currentWordIndex < _vocabulary.length) {
      ...
      _cardProvider.loadCard(word, card);
    }
  }
  ...
  void _submitAnswer(int rating) {
    ...
    // Use saved _cardProvider reference
  ...
  Future<void> _recordStudySessionOnExit() async {
    ...
    try {
      // Use saved _cardProvider and _progressProvider references instead of context.read()
      final cardProvider = _cardProvider;
      final progressProvider = _progressProvider;
  ...
  void _handleSessionComplete() async {
    ...
    try {
      // Use saved _progressProvider reference instead of context.read()
      final progressProvider = _progressProvider;
      ...
      Navigator.of(context).pushReplacement(
        MaterialPageRoute(
          builder: (context) => StudyResultPage(
            ...
            correctCards: _cardProvider.correctAnswers,
            wrongCards: _cardProvider.wrongAnswers,
            studyDuration: studyDuration,
            wrongWordIds: _cardProvider.wrongWordIds,
          ),
        ),
      );
    }
        ...
        actions: [
          ...
          TextButton(
            onPressed: () async {
              ...
              // Save progress before exiting using saved reference
              final progressProvider = _progressProvider;
